...
int_handler_syscall:
	// ebp/esp should already be good.
	cli
	pusha

	// call syscall
	// only use of this function.
	push %ebp
	mov %esp, %ebp

	// that pushed 36 bytes
	// push &arg
	lea 52(%esp), %eax
	push %eax
	// push syscall number
	...
	// fix state/interrupts, and return
	popa
	sti
	iret

// ignore undefined interrupts
.global int_handler_undef
int_handler_undef:
	iret
